...
RUN_TAG="${RUN_TAG:-v302_anchor_order_diagnostic_$(date +%Y%m%d_%H%M%S)}"
...
MIN_SOURCE_SAMPLES_PER_CLASS="${MIN_SOURCE_SAMPLES_PER_CLASS:-20}"
MIN_TARGET_SAMPLES_PER_CLASS="${MIN_TARGET_SAMPLES_PER_CLASS:-20}"
SHUFFLE_REPEATS="${SHUFFLE_REPEATS:-50}"
RANK_PERMUTATION_REPEATS="${RANK_PERMUTATION_REPEATS:-50}"
DTW_WINDOW_RATIO="${DTW_WINDOW_RATIO:-0.2}"
WRITE_SAMPLE_SEQUENCES="${WRITE_SAMPLE_SEQUENCES:-False}"

# Source experiment RUN_TAG embedded in output names, not the timestamped logs directory.
...
DA_ROWS="${DA_ROWS:-$ROOT_DIR/logs/v281_a_group_full_20260622_121128/full12/raw_strength_rows.tsv}"
...
run_job() {
  ...
  if [ ! -f "$checkpoint_path" ]; then
    printf "task\tsource\ttarget\tseed\tcheckpoint_config\tstatus\terror\n" > "$run_dir/failed_runs.tsv"
    printf "%s\t%s\t%s\t%s\t%s\tmissing_checkpoint\t%s\n" \
      "$task" "$source_dataset" "$target_dataset" "$seed" "$checkpoint_config" "$checkpoint_path" >> "$run_dir/failed_runs.tsv"
    echo "MISSING_CHECKPOINT|task=$task|seed=$seed|config=$checkpoint_config|path=$checkpoint_path"
    return 1
  fi
  CUDA_VISIBLE_DEVICES="$gpu" python analysis/v302_anchor_order_diagnostic.py \
    --mode run \
    --data_root "$DATA_ROOT" \
    ...
    --min_source_samples_per_class "$MIN_SOURCE_SAMPLES_PER_CLASS" \
    --min_target_samples_per_class "$MIN_TARGET_SAMPLES_PER_CLASS" \
    --shuffle_repeats "$SHUFFLE_REPEATS" \
    --rank_permutation_repeats "$RANK_PERMUTATION_REPEATS" \
    --dtw_window_ratio "$DTW_WINDOW_RATIO" \
    --write_sample_sequences "$WRITE_SAMPLE_SEQUENCES"
}
...
echo "DA_ROWS=$DA_ROWS"
echo "WRITE_SAMPLE_SEQUENCES=$WRITE_SAMPLE_SEQUENCES"
...
python "$ROOT_DIR/analysis/v302_anchor_order_diagnostic.py" \
  --mode aggregate \
  --input_root "$LOG_DIR/runs" \
  --output_dir "$LOG_DIR" \
  --da_rows "$DA_ROWS" \
  --write_sample_sequences "$WRITE_SAMPLE_SEQUENCES" || failed=1

END_TIME="$(date +%s)"
echo "TOTAL_RUNTIME_SECONDS=$((END_TIME - START_TIME))"
