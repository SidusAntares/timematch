...
MASTER_TAG="${MASTER_TAG:-v253fg_clean_divergence7_selection_only}"
...
run_variant() {
  ...
  run_variant_source_block 0 "france/30TXT/2017" $'france/31TCJ/2017' "$variant_tag"
  run_variant_source_block 1 "france/31TCJ/2017" $'denmark/32VNH/2017\naustria/33UVP/2017' "$variant_tag"
  run_variant_source_block 2 "denmark/32VNH/2017" $'france/31TCJ/2017\naustria/33UVP/2017' "$variant_tag"
  run_variant_source_block 3 "austria/33UVP/2017" $'france/31TCJ/2017\ndenmark/32VNH/2017' "$variant_tag"
  wait
  echo "===== Finished ${variant_tag}; logs: ${VARIANT_LOG_DIR} ====="
}

VARIANT_PLAN="${VARIANT_PLAN:-f,g}"
IFS=',' read -r -a VARIANTS <<< "$VARIANT_PLAN"
for variant in "${VARIANTS[@]}"; do
  case "$variant" in
    e)
      run_variant "v253e_clean_pureperturb_divergence7" "pure_perturbation"
      ;;
    f)
      run_variant "v253f_clean_pureperturb_late_reject_divergence7" "pure_perturbation_late_reject"
      ;;
    g)
      run_variant "v253g_clean_pureperturb_margin_tiebreak_divergence7" "pure_perturbation_margin_tiebreak"
      ;;
    *)
      echo "Unknown variant '${variant}'. Use VARIANT_PLAN=e, f, g, or comma-separated combinations such as f,g." >&2
      exit 1
      ;;
  esac
done

echo "Requested clean v2.5.3 divergence-7 selection-only runs finished: ${VARIANT_PLAN}"
