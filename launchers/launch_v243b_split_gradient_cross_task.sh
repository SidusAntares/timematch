...
RUN_TAG="${RUN_TAG:-v243b_split_gradient_cross_task}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/${RUN_TAG}_$(date +%Y%m%d_%H%M%S)}"
GPUS="${GPUS:-0 1 2 3}"
TASKS="${TASKS:-AT1_to_DK1,FR1_to_AT1}"
SEEDS="${SEEDS:-1 2 3}"
...
# Keep DA-stage source structure off. This experiment validates source-stage
# split-gradient responsibility across tasks.
...
add_task_seed_jobs() {
  ...
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "trainable_s003_both_global_${COMPACT_TAG}" \
    ...
    "Split-gradient candidate: raw compactness updates encoder, reshaped compactness updates reshaper."
}
...
run_worker() {
  ...
  while IFS=$'\t' read -r task source_dataset target_dataset seed config reshaper strength trainable init_seed reg_trade_off dual_relation intra structure_target detach_features est_weight; do
    local tag="v243b_splitgrad_${task}_seed${seed}_${config}"
    local log_file="$LOG_DIR/gpu${gpu}_${task}_seed${seed}_${config}.log"
    echo "START|gpu=$gpu|task=$task|seed=$seed|config=$config|target=$structure_target|intra=$intra|strength=$strength|trainable=$trainable|init=$init_seed|log=$log_file"
    (
      export SEED="$seed"
