...
RUN_TAG="${RUN_TAG:-v253c_trajectory_selection_quickcheck}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/${RUN_TAG}_$(date +%Y%m%d_%H%M%S)}"

mkdir -p "$LOG_DIR"

export DATA_ROOT="${DATA_ROOT:-/data/user/DBL/timematch_data}"
export RESHAPER_TAG="${RESHAPER_TAG:-v253c_trajectory_s010_rel003}"
...
echo "v2.5.3c trajectory selection quickcheck logs:"
