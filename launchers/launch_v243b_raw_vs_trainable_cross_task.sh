...
RUN_TAG="${RUN_TAG:-v243b_raw_vs_trainable_cross_task}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/${RUN_TAG}_$(date +%Y%m%d_%H%M%S)}"
GPUS="${GPUS:-0 1 2 3}"
TASKS="${TASKS:-DK1_to_FR1}"
...
# Keep DA-stage source structure off. This experiment isolates the
# raw-compactness vs trainable-reshaper interaction under the same source-stage
# training口径 as previous v2.7.5 runs.
...
add_task_seed_jobs() {
  ...
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "trainable_s003_raw_global_${COMPACT_TAG}" \
    ...
    "Trainable near-identity reshaper plus raw encoder compactness."
}
...
run_worker() {
  ...
  while IFS=$'\t' read -r task source_dataset target_dataset seed config reshaper strength trainable init_seed reg_trade_off dual_relation intra structure_target detach_features est_weight; do
    local tag="v243b_rawtrain_${task}_seed${seed}_${config}"
