...
RUN_TAG="${RUN_TAG:-v25_theory_followup_probe_${BATCH_STAMP}}"
...
# Theory follow-up tasks:
# - FR2->DK1: large old v2.4.3b gain; checks if local compactness explains a hard target.
# - DK1->FR2: source DK to France, tests cross-country asymmetry without repeating DK1->FR1.
# - AT1->DK1: strong old gain; tests whether compactness works when Austria is source.
# - FR1->AT1: high-baseline task; checks whether structure still helps when baseline is already strong.
TASK_SPECS="${TASK_SPECS:-FR2_to_DK1|france/31TCJ/2017|denmark/32VNH/2017,DK1_to_FR2|denmark/32VNH/2017|france/31TCJ/2017,AT1_to_DK1|austria/33UVP/2017|denmark/32VNH/2017,FR1_to_AT1|france/30TXT/2017|austria/33UVP/2017}"
...
#
# The variants deliberately avoid old trend/boundary/dynamics-heavy settings:
# - global_k1_intra: no-seg compactness view.
# - uniform_k5_intra: fixed local compactness view.
# - doy_k5_intra: irregular-observation compactness view.
# - uniform_k5_full_light: compactness plus weak transition/semantic preservation.
# - uniform_k5_no_dual: same local compactness, but no dual-path preservation.
VARIANT_SPECS="${VARIANT_SPECS:-global_k1_intra:uniform:segment_boundary_window_residual:1:1:5.0:0.00:0.00:0.00:0.00:meanmax:cosine:1.00:0.03,uniform_k5_intra:uniform:segment_boundary_window_residual:5:5:1.0:0.00:0.00:0.00:0.00:meanmax:cosine:1.00:0.03,doy_k5_intra:doy_gap:segment_boundary_window_residual:5:5:1.0:0.00:0.00:0.00:0.00:meanmax:cosine:1.00:0.03,uniform_k5_full_light:uniform:segment_boundary_window_residual:5:5:1.0:0.05:0.02:0.00:0.00:meanmax:cosine:1.00:0.03,uniform_k5_no_dual:uniform:segment_boundary_window_residual:5:5:1.0:0.00:0.00:0.00:0.00:meanmax:cosine:0.00:0.00}"
...
run_one() {
  ...
  echo "START|${job_tag}|gpu=${gpu_id}|loss=${loss_version}|partition=${partition_mode}|intra=${intra_weight}|trend=${trend_weight}|segment_inter=${segment_inter_weight}|boundary=${boundary_weight}|dual_cls=${dual_cls}|dual_relation=${dual_relation}"
  (
...
echo "v2.5 theory follow-up probe finished."
echo "Logs saved under: $LOG_DIR"
