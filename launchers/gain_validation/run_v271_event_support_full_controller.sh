...
RUN_TAG="${RUN_TAG:-v271_event_support_full_${STAMP}}"
...
configure_structure() {
  ...
  local event_mode="event"
  local event_weight="0.0"
  ...
  case "$mode" in
    G_global)
      ...
      ;;
    G_event_support)
      loss_version="v271_global_event_support"
      event_mode="event"
      event_weight="$EVENT_SUPPORT_TRADE_OFF"
      ;;
    G_random_event)
      loss_version="v271_global_event_support"
      event_mode="matched_random"
      event_weight="$EVENT_SUPPORT_TRADE_OFF"
      ;;
    *)
...
run_pipeline() {
  ...
  local exp_base="${dataset_key}_${task_key}_${mode_key}_t${trend_weight}_ew${EVENT_SUPPORT_TRADE_OFF}_ec${EVENT_SUPPORT_COUNT}_sr${EVENT_SUPPORT_SIGMA_RATIO}"
  exp_base="${exp_base//./}"
...
wait
echo "v2.7.1 event-support full controller finished."
echo "Summary: $SUMMARY_FILE"
