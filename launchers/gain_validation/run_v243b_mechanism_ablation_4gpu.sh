...
RUN_TAG="${RUN_TAG:-v243b_mech_ablation_${BATCH_STAMP}}"
...
ABLATION_MODES="${ABLATION_MODES:-A_exact,B_no_da_shaping,C_no_boundary,D_global_like}"
...
IFS=',' read -r -a MODE_ITEMS <<< "$ABLATION_MODES"
...
mode_spec() {
  ...
  case "$mode" in
    A_exact)
      echo "segment_boundary_window_residual doy_gap 5 $base_trend $base_segment_inter $base_boundary 1.0"
      ;;
    B_no_da_shaping)
      echo "segment_boundary_window_residual doy_gap 5 $base_trend $base_segment_inter $base_boundary 0.0"
      ;;
    C_no_boundary)
      echo "segment_boundary_window_residual doy_gap 5 $base_trend $base_segment_inter 0.0 1.0"
      ;;
    D_global_like)
      echo "trend_residual uniform 1 $base_trend 0.0 0.0 1.0"
      ;;
    *)
...
run_one() {
  ...
  local loss_version partition_mode phase_count trend_weight segment_inter_weight boundary_weight da_structure_weight
  read -r loss_version partition_mode phase_count trend_weight segment_inter_weight boundary_weight da_structure_weight \
    <<< "$(mode_spec "$mode" "$base_trend" "$base_segment_inter" "$base_boundary")"
  ...
  variant_name="v243b_${mode}_${task_name}_t${trend_weight}_si${segment_inter_weight}_b${boundary_weight}_da${da_structure_weight}"
  ...
  echo "START|${mode}|${task_name}|gpu=${gpu_id}|loss=${loss_version}|partition=${partition_mode}|phase_count=${phase_count}|trend=${trend_weight}|segment_inter=${segment_inter_weight}|boundary=${boundary_weight}|da_structure=${da_structure_weight}|log=${log_file}"
  (
    common_args "$source_dataset" "$source_dataset" "$loss_version" "$partition_mode" "$phase_count" "$trend_weight" "$segment_inter_weight" "$boundary_weight"
    echo "MECH_SOURCE_START|${mode}|${task_name}|${source_exp}"
    ...
    if [ "$EVAL_SOURCE_ON_TARGET" = "true" ]; then
      common_args "$source_dataset" "$target_dataset" "$loss_version" "$partition_mode" "$phase_count" "$trend_weight" "$segment_inter_weight" "$boundary_weight"
      echo "MECH_SOURCE_TARGET_EVAL|${mode}|${task_name}|${source_exp}|target=${target_dataset}"
      CUDA_VISIBLE_DEVICES="$gpu_id" python train.py \
        "${COMMON_ARGS[@]}" \
        --output_dir "$OUT_ROOT" \
        --tensorboard_log_dir "$RUN_ROOT" \
    ...
    common_args "$source_dataset" "$target_dataset" "$loss_version" "$partition_mode" "$phase_count" "$trend_weight" "$segment_inter_weight" "$boundary_weight"
    echo "MECH_DA_START|${mode}|${task_name}|${da_exp}|weights=${source_out}"
    CUDA_VISIBLE_DEVICES="$gpu_id" python train.py \
      "${COMMON_ARGS[@]}" \
      --output_dir "$OUT_ROOT" \
      --tensorboard_log_dir "$RUN_ROOT" \
...
echo "All v2.4.3b mechanism ablation jobs finished."
