...
MASTER_TAG="${MASTER_TAG:-v321_stage3_probe_$(date +%Y%m%d_%H%M%S)}"
LOG_DIR="${LOG_DIR:-${ROOT}/logs/${MASTER_TAG}}"
REF_ROOT="${REF_ROOT:-${ROOT}/outputs/${MASTER_TAG}_refs}"
GPUS="${GPUS:-0 1 2 3}"
TASKS="${TASKS:-FR1_to_FR2,FR2_to_AT1,FR2_to_FR1,AT1_to_DK1}"
CONFIGS="${CONFIGS:-base,smooth_base,global_only,residual}"
SEED="${SEED:-1}"
EPOCHS="${EPOCHS:-20}"
...
LOCAL_SHIFT_SOURCE_KIND="${LOCAL_SHIFT_SOURCE_KIND:-smooth_if_available}"
...
task_spec() {
  case "$1" in
    ...
    FR1_to_DK1) echo "france/30TXT/2017 denmark/32VNH/2017 30TXT" ;;
    FR1_to_AT1) echo "france/30TXT/2017 austria/33UVP/2017 30TXT" ;;
    FR2_to_FR1) echo "france/31TCJ/2017 france/30TXT/2017 31TCJ" ;;
    FR2_to_DK1) echo "france/31TCJ/2017 denmark/32VNH/2017 31TCJ" ;;
    FR2_to_AT1) echo "france/31TCJ/2017 austria/33UVP/2017 31TCJ" ;;
    DK1_to_FR1) echo "denmark/32VNH/2017 france/30TXT/2017 32VNH" ;;
    DK1_to_FR2) echo "denmark/32VNH/2017 france/31TCJ/2017 32VNH" ;;
    DK1_to_AT1) echo "denmark/32VNH/2017 austria/33UVP/2017 32VNH" ;;
    AT1_to_FR1) echo "austria/33UVP/2017 france/30TXT/2017 33UVP" ;;
    AT1_to_FR2) echo "austria/33UVP/2017 france/31TCJ/2017 33UVP" ;;
...
choose_local_source_kind() {
  local plain_dir="$1"
  local smooth_dir="$2"
  case "${LOCAL_SHIFT_SOURCE_KIND}" in
    plain) echo "plain" ;;
    smooth) echo "smooth" ;;
    smooth_if_available)
      if [[ -n "${smooth_dir}" && -f "${smooth_dir}/fold_0/model.pt" ]]; then
        echo "smooth"
      else
        echo "plain"
      fi
      ;;
    *) echo "ERROR unknown LOCAL_SHIFT_SOURCE_KIND=${LOCAL_SHIFT_SOURCE_KIND}" >&2; return 2 ;;
  esac
}
...
for raw_task in "${TASK_NAMES[@]}"; do
  ...
  plain_dir="$(find_source_dir "${task}" "${source_tile}" plain || true)"
  ...
  for raw_config in "${CONFIG_NAMES[@]}"; do
    ...
    case "${config}" in
      base)
        if [[ -n "${plain_dir}" && -f "${plain_dir}/fold_0/model.pt" ]]; then
          append_job "${task}" "${config}" "${source_dataset}" "${target_dataset}" "plain" "${plain_dir}" ""
        else
          echo "SKIP|task=${task}|config=${config}|reason=missing_plain_source"
        fi
        ;;
      smooth_base)
        if [[ -n "${smooth_dir}" && -f "${smooth_dir}/fold_0/model.pt" ]]; then
          ...
          echo "SKIP|task=${task}|config=${config}|reason=missing_smooth_source"
        fi
        ;;
      global_only|residual)
        local_kind="$(choose_local_source_kind "${plain_dir}" "${smooth_dir}")" || exit 2
        if [[ "${local_kind}" == "smooth" ]]; then
          local_dir="${smooth_dir}"
        else
          local_dir="${plain_dir}"
        fi
        if [[ -z "${local_dir}" || ! -f "${local_dir}/fold_0/model.pt" ]]; then
          echo "SKIP|task=${task}|config=${config}|reason=missing_${local_kind}_source"
          continue
        fi
        ref_path="$(build_reference_if_needed "${task}" "${source_dataset}" "${local_kind}" "${local_dir}")" || continue
        append_job "${task}" "${config}" "${source_dataset}" "${target_dataset}" "${local_kind}" "${local_dir}" "${ref_path}"
        ;;
      *)
...
run_job() {
  ...
  start_time="$(date +%s)"
  echo "START|task=${task}|config=${config}|source_kind=${source_kind}|gpu=${gpu}|log=${run_log}"

  if [[ "${DRY_RUN}" == "True" ]]; then
    status=0
  elif [[ "${config}" == "base" || "${config}" == "smooth_base" ]]; then
  ...
  end_time="$(date +%s)"
  if [[ "${config}" == "base" || "${config}" == "smooth_base" ]]; then
    local_shift_log=""
  else
    diagnostic_log=""
  fi
...
python "${ROOT}/analysis/summarize_v321_stage3_probe.py" \
  --log_dir "${LOG_DIR}" \
  --output "${SUMMARY}" || true
