...
MASTER_TAG="${MASTER_TAG:-v321_stage3c_equivalence_fix_$(date +%Y%m%d_%H%M%S)}"
...
REPORT="${ROOT}/analysis/v321_stage3c_equivalence_fix_report.md"
EQUIV_DEBUG_STEPS="${EQUIV_DEBUG_STEPS:-3}"
: > "${JOBS}"
...
run_job() {
  ...
  local equiv_debug_log="${LOG_DIR}/equiv_debug_${task}_${config}.tsv"
  ...
  if [[ "${DRY_RUN}" == "True" ]]; then
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" python "${ROOT}/train.py" \
      ...
      --timematch_equiv_debug_steps "${EQUIV_DEBUG_STEPS}" \
      --timematch_equiv_debug_path "${equiv_debug_log}" \
      --balance_source True \
      > "${run_log}" 2>&1
    status=$?
    local_shift_log=""
    ...
    CUDA_VISIBLE_DEVICES="${gpu}" python "${ROOT}/train.py" \
      ...
      --local_shift_equiv_debug_steps "${EQUIV_DEBUG_STEPS}" \
      --local_shift_equiv_debug_path "${equiv_debug_log}" \
      --balance_source True \
      > "${run_log}" 2>&1
    status=$?
    diagnostic_log=""
