...
RUN_TAG="${RUN_TAG:-v27a_elastic_structure_probe_${STAMP}}"
...
# name:enabled:radius:temperature:blend
VARIANTS=(${VARIANTS:-original:False:0:0.10:1.00 elastic_r2_t010:True:2:0.10:1.00 elastic_r3_t010:True:3:0.10:1.00})
...
run_remote_task() {
  ...
  local elastic_enabled="$6"
  local elastic_radius="$7"
  local elastic_temperature="$8"
  local elastic_blend="$9"
  local gpu="${10}"
  local tag="remote_${src_alias}_to_${tgt_alias}_${variant_name}"
  local source_exp="${tag}_v27a_source_${STAMP}"
  local da_exp="${tag}_v27a_timematch_${STAMP}"
  local source_log="${LOG_ROOT}/${source_exp}.log"
  local da_log="${LOG_ROOT}/${da_exp}.log"
  local source_out="${OUT_ROOT}/${source_exp}"

  echo "[v2.7a] remote ${src_alias}->${tgt_alias} variant=${variant_name} source on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py \
    ...
    "${common_structure_args[@]}" \
    "${remote_partition_args[@]}" \
    --source_structure_elastic_compactness "${elastic_enabled}" \
    --source_structure_elastic_radius "${elastic_radius}" \
    --source_structure_elastic_temperature "${elastic_temperature}" \
    --source_structure_elastic_blend "${elastic_blend}" \
    sourcephasecompact \
    > "${source_log}" 2>&1

  echo "[v2.7a] remote ${src_alias}->${tgt_alias} variant=${variant_name} timematch on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py \
    --data_root "${REMOTE_DATA_ROOT}" \
    --closed_set True \
    --with_shift_aug False \
  ...
  echo -e "remote\t${src_alias}->${tgt_alias}\t${variant_name}\t${elastic_enabled}\t${elastic_radius}\t${elastic_temperature}\t${elastic_blend}\t${source_log}\t${da_log}" >> "${LOG_ROOT}/task_logs.tsv"
}
...
run_har_task() {
  ...
  local elastic_enabled="$5"
  local elastic_radius="$6"
  local elastic_temperature="$7"
  local elastic_blend="$8"
  local gpu="$9"
  ...
  echo "[v2.7a] ${dataset} ${src}->${tgt} variant=${variant_name} source on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py \
    --dataset_type har \
  ...
  echo "[v2.7a] ${dataset} ${src}->${tgt} variant=${variant_name} timematch on GPU ${gpu}"
  CUDA_VISIBLE_DEVICES="${gpu}" python train.py \
    --dataset_type har \
    --har_dataset_name "${dataset}" \
    --data_root "${data_root}" \
  ...
  echo -e "${dataset}\t${src}->${tgt}\t${variant_name}\t${elastic_enabled}\t${elastic_radius}\t${elastic_temperature}\t${elastic_blend}\t${source_log}\t${da_log}" >> "${LOG_ROOT}/task_logs.tsv"
}

echo -e "dataset\ttask\tvariant\telastic_enabled\telastic_radius\telastic_temperature\telastic_blend\tsource_log\tda_log" > "${LOG_ROOT}/task_logs.tsv"
echo "v2.7a elastic source structure probe"
echo "Logs: ${LOG_ROOT}"
...
for variant_spec in "${VARIANTS[@]}"; do
  IFS=':' read -r variant_name elastic_enabled elastic_radius elastic_temperature elastic_blend <<< "${variant_spec}"
  for task in "${REMOTE_TASKS[@]}"; do
    ...
    run_remote_task "${src_alias}" "${tgt_alias}" "${source_dataset}" "${target_dataset}" "${variant_name}" "${elastic_enabled}" "${elastic_radius}" "${elastic_temperature}" "${elastic_blend}" "${gpu}" &
    job_idx=$((job_idx + 1))
  done
  for task in "${HAR_TASKS[@]}"; do
    ...
    wait_for_slot
    gpu="${GPUS[$((job_idx % ${#GPUS[@]}))]}"
    run_har_task "${dataset}" "${src}" "${tgt}" "${variant_name}" "${elastic_enabled}" "${elastic_radius}" "${elastic_temperature}" "${elastic_blend}" "${gpu}" &
    job_idx=$((job_idx + 1))
  done
done

wait
echo "v2.7a elastic source structure probe finished."
echo "Logs: ${LOG_ROOT}"
