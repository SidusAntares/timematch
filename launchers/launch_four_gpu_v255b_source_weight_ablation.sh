#!/bin/bash

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
IDEA_DIR="$SCRIPT_DIR/ideas"
RUN_TAG="${RUN_TAG:-v255b_source_weight_ablation}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/${RUN_TAG}_$(date +%Y%m%d_%H%M%S)}"
VARIANTS="${VARIANTS:-baseline,transition_light}"

mkdir -p "$LOG_DIR"

export DATA_ROOT="${DATA_ROOT:-/data/user/DBL/timematch_data}"
export SOURCE_SKIP_TRAIN="${SOURCE_SKIP_TRAIN:-0}"
export SOURCE_PRETRAIN_EPOCHS="${SOURCE_PRETRAIN_EPOCHS:-100}"
export SOURCE_CHECKPOINT_EPOCHS="${SOURCE_CHECKPOINT_EPOCHS:-30,50,70,100}"
export SOURCE_WEIGHTS_CHECKPOINTS="${SOURCE_WEIGHTS_CHECKPOINTS:-checkpoints/epoch_30.pt,checkpoints/epoch_50.pt,checkpoints/epoch_70.pt,checkpoints/epoch_100.pt}"
export SOURCE_CHECKPOINT_DIRNAME="${SOURCE_CHECKPOINT_DIRNAME:-checkpoints}"
export TIMEMATCH_EPOCHS="${TIMEMATCH_EPOCHS:-20}"
export NUM_WORKERS="${NUM_WORKERS:-8}"
export BATCH_SIZE="${BATCH_SIZE:-128}"

export SOURCE_FEATURE_RESHAPER="${SOURCE_FEATURE_RESHAPER:-residual_temporal_conv}"
export SOURCE_FEATURE_RESHAPER_STRENGTH="${SOURCE_FEATURE_RESHAPER_STRENGTH:-0.10}"
export SOURCE_FEATURE_RESHAPER_KERNEL_SIZE="${SOURCE_FEATURE_RESHAPER_KERNEL_SIZE:-3}"
export SOURCE_FEATURE_DUAL_CLS_TRADE_OFF="${SOURCE_FEATURE_DUAL_CLS_TRADE_OFF:-1.00}"

export SOURCE_PHASE_PARTITION_MODE="${SOURCE_PHASE_PARTITION_MODE:-doy_gap}"
export SOURCE_PHASE_COUNT="${SOURCE_PHASE_COUNT:-5}"
export SOURCE_SEGMENT_PARTITION_MODE="${SOURCE_SEGMENT_PARTITION_MODE:-$SOURCE_PHASE_PARTITION_MODE}"
export SOURCE_SEGMENT_COUNT="${SOURCE_SEGMENT_COUNT:-$SOURCE_PHASE_COUNT}"
export SOURCE_PHASE_GAP_THRESHOLD="${SOURCE_PHASE_GAP_THRESHOLD:-45}"
export SOURCE_PHASE_MIN_POINTS="${SOURCE_PHASE_MIN_POINTS:-3}"
export SOURCE_PHASE_MAX_POINTS="${SOURCE_PHASE_MAX_POINTS:-8}"
export SOURCE_PHASE_MAX_SPAN="${SOURCE_PHASE_MAX_SPAN:-120}"
export SOURCE_PHASE_MIN_SAMPLE_POINTS="${SOURCE_PHASE_MIN_SAMPLE_POINTS:-2}"

export SOURCE_STRUCTURE_LOSS_VERSION="${SOURCE_STRUCTURE_LOSS_VERSION:-segment_boundary_window_residual}"
export SOURCE_STRUCTURE_INTRA_TRADE_OFF="${SOURCE_STRUCTURE_INTRA_TRADE_OFF:-1.0}"
export SOURCE_STRUCTURE_AMPLITUDE_TRADE_OFF="${SOURCE_STRUCTURE_AMPLITUDE_TRADE_OFF:-0.00}"
export SOURCE_STRUCTURE_INTERPHASE_TRADE_OFF="${SOURCE_STRUCTURE_INTERPHASE_TRADE_OFF:-0.00}"
export SOURCE_STRUCTURE_SHAPE_TRADE_OFF="${SOURCE_STRUCTURE_SHAPE_TRADE_OFF:-0.00}"
export SOURCE_STRUCTURE_SEASON_TRADE_OFF="${SOURCE_STRUCTURE_SEASON_TRADE_OFF:-0.00}"
export SOURCE_STRUCTURE_BOUNDARY_WINDOW_SIZE="${SOURCE_STRUCTURE_BOUNDARY_WINDOW_SIZE:-2}"

set_variant_weights() {
  local variant="$1"

  case "$variant" in
    baseline)
      export SOURCE_STRUCTURE_TREND_TRADE_OFF="0.05"
      export SOURCE_STRUCTURE_SEGMENT_INTER_TRADE_OFF="0.02"
      export SOURCE_STRUCTURE_BOUNDARY_WINDOW_TRADE_OFF="0.20"
      export SOURCE_FEATURE_RESHAPER_REG_TRADE_OFF="0.05"
      export SOURCE_FEATURE_DUAL_RELATION_TRADE_OFF="0.03"
      ;;
    transition_light)
      export SOURCE_STRUCTURE_TREND_TRADE_OFF="0.05"
      export SOURCE_STRUCTURE_SEGMENT_INTER_TRADE_OFF="0.01"
      export SOURCE_STRUCTURE_BOUNDARY_WINDOW_TRADE_OFF="0.10"
      export SOURCE_FEATURE_RESHAPER_REG_TRADE_OFF="0.05"
      export SOURCE_FEATURE_DUAL_RELATION_TRADE_OFF="0.03"
      ;;
    structure_light)
      export SOURCE_STRUCTURE_TREND_TRADE_OFF="0.025"
      export SOURCE_STRUCTURE_SEGMENT_INTER_TRADE_OFF="0.01"
      export SOURCE_STRUCTURE_BOUNDARY_WINDOW_TRADE_OFF="0.10"
      export SOURCE_FEATURE_RESHAPER_REG_TRADE_OFF="0.025"
      export SOURCE_FEATURE_DUAL_RELATION_TRADE_OFF="0.015"
      ;;
    *)
      echo "Unknown v2.5.5b variant: $variant" >&2
      echo "Allowed variants: baseline,transition_light,structure_light" >&2
      exit 2
      ;;
  esac
}

run_pair() {
  local gpu_id="$1"
  local variant="$2"
  local source_dataset="$3"
  local target_dataset="$4"
  local source_tile
  local target_tile
  source_tile="$(echo "$source_dataset" | cut -d'/' -f2)"
  target_tile="$(echo "$target_dataset" | cut -d'/' -f2)"
  local log_file="$LOG_DIR/gpu${gpu_id}_${source_tile}_to_${target_tile}_${variant}.log"

  (
    CUDA_VISIBLE_DEVICES="$gpu_id" \
      VARIANT_NAME="$variant" \
      SOURCE="$source_dataset" \
      TARGETS_BLOCK="$target_dataset" \
      SOURCE_MODEL_TAG="v255b_${variant}_s010_rel003_ckptbank" \
      RESHAPER_TAG="v255b_${variant}_allckpt_s010_rel003" \
      TARGET_MODEL_TAG="v255b_${variant}_allckpt_s010_rel003" \
      bash "$IDEA_DIR/run_timematch_closed_set_sourcephasecompact_v255_allcheckpoints_block.sh"
  ) > "$log_file" 2>&1 &
}

IFS=',' read -r -a VARIANT_ITEMS <<< "$VARIANTS"
for variant in "${VARIANT_ITEMS[@]}"; do
  variant="$(echo "$variant" | xargs)"
  if [ -z "$variant" ]; then
    continue
  fi

  echo "===== Starting v2.5.5b source weight ablation variant: $variant ====="
  set_variant_weights "$variant"

  run_pair 0 "$variant" "france/30TXT/2017" "austria/33UVP/2017"
  run_pair 1 "$variant" "france/31TCJ/2017" "denmark/32VNH/2017"
  run_pair 2 "$variant" "denmark/32VNH/2017" "austria/33UVP/2017"
  run_pair 3 "$variant" "austria/33UVP/2017" "denmark/32VNH/2017"

  wait
  echo "===== Finished v2.5.5b source weight ablation variant: $variant ====="
done

echo "Logs saved to: $LOG_DIR"
echo "v2.5.5b source-weight ablation logs:"
for variant in "${VARIANT_ITEMS[@]}"; do
  variant="$(echo "$variant" | xargs)"
  if [ -z "$variant" ]; then
    continue
  fi
  echo "  $LOG_DIR/gpu0_30TXT_to_33UVP_${variant}.log"
  echo "  $LOG_DIR/gpu1_31TCJ_to_32VNH_${variant}.log"
  echo "  $LOG_DIR/gpu2_32VNH_to_33UVP_${variant}.log"
  echo "  $LOG_DIR/gpu3_33UVP_to_32VNH_${variant}.log"
done
