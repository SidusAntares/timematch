...
RUN_TAG="${RUN_TAG:-dir1_monotonic_warp_reshaper_quickcheck}"
LOG_DIR="${LOG_DIR:-$ROOT_DIR/logs/${RUN_TAG}_$(date +%Y%m%d_%H%M%S)}"

mkdir -p "$LOG_DIR"

export DATA_ROOT="${DATA_ROOT:-/data/user/DBL/timematch_data}"
export VARIANT_NAME="${VARIANT_NAME:-dir1_monotonic_warp_reshaper}"
...
export SOURCE_FEATURE_RESHAPER="${SOURCE_FEATURE_RESHAPER:-monotonic_warp_residual_temporal_conv}"
export SOURCE_FEATURE_RESHAPER_STRENGTH="${SOURCE_FEATURE_RESHAPER_STRENGTH:-0.08}"
...
export SOURCE_MODEL_TAG="${SOURCE_MODEL_TAG:-dir1_monotonic_warp_reshaper_s010_rel003_ckptbank}"
export RESHAPER_TAG="${RESHAPER_TAG:-dir1_monotonic_warp_reshaper_allckpt_s010_rel003}"
