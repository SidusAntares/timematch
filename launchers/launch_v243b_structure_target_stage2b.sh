...
RUN_TAG="${RUN_TAG:-v243b_structure_target_stage2b}"
...
# Keep DA-stage source structure off. This experiment isolates source-stage
# feature target effects.
...
add_task_seed_jobs() {
  ...
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "raw_global_${COMPACT_TAG}" \
    "raw_compact" "none" "0.00" "False" "-1" "0.00" "0.00" "$COMPACT_WEIGHT" "raw" "False" "$est_weight" \
    "Global compactness directly on raw encoder features."
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "raw_global_${COMPACT_TAG}_detached" \
    "raw_compact_detached" "none" "0.00" "False" "-1" "0.00" "0.00" "$COMPACT_WEIGHT" "raw" "True" "$est_weight" \
    "Detached raw compactness control; should not update encoder."

  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "strength0_dualcls_rel003" \
    "strength0_dualpath" "residual_temporal_conv" "0.00" "False" "101" "0.00" "0.03" "0.0" "auto" "False" "$est_weight" \
    "Identity reshaper with dual classifier path and relation loss."
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "strength0_raw_global_${COMPACT_TAG}" \
  ...
  add_config "$task" "$source_dataset" "$target_dataset" "$seed" "trainable_s003_raw_global_${COMPACT_TAG}" \
    "trainable_s003_plus_raw_compact" "residual_temporal_conv" "0.03" "True" "101" "0.00" "0.03" "$COMPACT_WEIGHT" "raw" "False" "$est_weight" \
    "Trainable reshaper plus raw encoder compactness."
...
run_worker() {
  ...
  while IFS=$'\t' read -r task source_dataset target_dataset seed config reshaper strength trainable init_seed reg_trade_off dual_relation intra structure_target detach_features est_weight; do
    local tag="v243b_stage2b_${task}_seed${seed}_${config}"
...
python "$ROOT_DIR/analysis/summarize_v243b_structure_target_stage2b.py" "$LOG_DIR" || true
