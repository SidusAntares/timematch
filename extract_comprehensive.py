...
log_dir = r"C:\Code\dev\PythonProject	imematch\logs255b_source_weight_ablation_20260514_134715"
...
for r in all_results:
    ...
    rows.append({
        "timestamp": r["timestamps"][0] if r["timestamps"] else "N/A",
        "file": r["file"],
        "variant": r["variant"],
        "weight_params": r["weight_params"],
        "total_lines": r["total_lines"],
        "source_completed": r["source_completed"],
        "source_val_count": len(r["source_only_val"]),
        "source_best_val_f1": r["source_best_val_f1"],
        "source_last_val_f1": r["source_last_val_f1"],
        "source_last_val_acc": r["source_last_val_acc"],
        "timematch_started": r["timematch_started"],
        "timematch_max_epoch": r["timematch_max_epoch"],
        "timematch_last_loss": r["timematch_last_loss"],
        "da_checkpoints_done": len(r["source_checkpoints_completed"]),
        "da_test_results": da_test if da_test else "N/A",
        "tm_last_val_f1": r["tm_last_val_f1"],
        "tm_last_val_acc": r["tm_last_val_acc"],
        "detected_phases": {"source_training_completed": r["source_completed"], "timematch_running": r["timematch_started"], "da_completed_checkpoints": len(r["source_checkpoints_completed"])}
    })

summary_parts = ["Extracted data from %d structure_light.log files." % len(files), "All weight config: trend=0.025, segment_inter=0.01, boundary_window=0.10, reshaper_reg=0.025, dual_relation=0.015."]
for r in rows:
    summary_parts.append("%s: src_epochs=%d, best_f1=%s, tm_max_epoch=%d, da_done=%d" % (r["file"], r["source_val_count"], str(r["source_best_val_f1"]), r["timematch_max_epoch"], r["da_checkpoints_done"]))
    if r["da_test_results"] != "N/A":
        summary_parts.append("  DA test: %s" % str(r["da_test_results"]))

discrepancies = []
if rows[0]["source_completed"] == False:
    discrepancies.append("gpu0 source training incomplete: stopped at 75/100 epochs (only 1521 lines). Never reached TimeMatch.")

output = {"files_found": file_names, "extracted_rows": rows, "oracle_alignment": {"matched": 0, "mismatched": 0, "note": "Skipped"}, "discrepancies": discrepancies, "summary": " ".join(summary_parts)}
