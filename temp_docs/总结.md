...
这也意味着，source-side structure shaping 不是一个边缘技巧，而是当前整条线里最可靠的正收益来源。
...
- `V5` 这种直接对伪标签类结构做 compactness 的方案风险很高
- `V6` 虽然比 `V5` 更合理，但单独使用时仍然偏弱
...
### 3.3 `V7` 和 `V8` 共同证明：target-side structure regularization 需要 source foundation
...
它的实验意义不在于“数值大幅超过第七版”，而在于它把一个关键问题说清楚了：
...
1. 第八版已经完全脱离第六版的弱版本区间
2. 第八版与第七版几乎处于同一强度档次
3. target 端是否使用伪标签，并不是唯一决定因素
4. source-side compactness 才是 target structure line 能否成立的重要前提之一

因此，第八版最重要的价值是：

> **它是一个非常成功的消融版本。**
> 它证明了第七版的效果并不完全依赖 target 伪标签结构约束。  
> 当 source foundation 已经建立后，第六版那种无监督 target consistency 也能够进入强版本区间。
