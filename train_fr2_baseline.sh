...
# Multi-source baseline TimeMatch script.
# This script does NOT enable PRA.
# It can reuse or run the original baseline experiment names:
#   source models: pseltae_FR1 / pseltae_FR2 / pseltae_DK1 / pseltae_AT1
#   DA models:     timematch_<SRC>_to_<TGT>
# and will iterate over every source-target pair with source != target.
...
write_summary_csv() {
    ...
    {
        echo "source,target,seed,source_experiment,timematch_experiment,source_accuracy,timematch_accuracy,timematch_minus_source_accuracy,source_macro_f1,timematch_macro_f1,timematch_minus_source_macro_f1"
        echo "$source_path,$target_path,$seed,$source_exp,$timematch_exp,$source_accuracy,$timematch_accuracy,$delta_acc,$source_macro_f1,$timematch_macro_f1,$delta_f1"
    } > "$csv_path"
...
run_experiment() {
    ...
    echo "--------------------------------------------------"
    echo "[INFO] Starting baseline flow: source=$source_path, target=$target_path, seed=$seed"
...
echo "[SUCCESS] Finished multi-source baseline experiments."
